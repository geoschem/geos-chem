...
!  12 Jun 2020 - L. Jaegle & J. Huang - Add blowing snow sea salt over polar regions
!EOP
!------------------------------------------------------------------------------
!
! !PRIVATE TYPES:
...
!       (jaegle 5/11/11)
!  (8 ) Add blowing snow sea salt emissions (jaegle, huang, 04/09/20)
...
!
    ...
    REAL*8, PARAMETER      :: LS = 2839d3    ! Latent heat of sublimation @ T=-30C (J/kg). Varies very little with Temperature
    ...
    IF ( Inst%EmitSnowSS ) THEN
      ...
      CALL HCO_EvalFld ( am_I_Root, HcoState, 'MULTISEAICE', &
                              MULTI, RC )
      IF ( RC /= HCO_SUCCESS ) THEN
