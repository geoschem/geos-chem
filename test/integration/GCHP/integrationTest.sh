...
if [[ "x${activeCondaEnv}" == "xtrue" ]]; then
    echo "ERROR: Integration tests cannot submitted when a conda environment is active!"
    echo "This may result in GCHP being linked against the wrong libraries."
    echo "Please deactivate the environment and submit the integration tests again."
    exit 1
fi
