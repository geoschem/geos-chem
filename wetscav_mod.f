! $Id: wetscav_mod.f,v 1.15 2005/03/29 15:52:45 bmy Exp $
      MODULE WETSCAV_MOD
!
!******************************************************************************
!  Module WETSCAV_MOD contains arrays for used in the wet scavenging of
!  tracer in cloud updrafts, rainout, and washout. (bmy, 2/28/00, 3/7/05)
!
!  Module Variables:
!  ============================================================================
...
!------------------------------------------------------------------------------
! Prior to 3/7/05:
! Now use analytic function for computational efficiency (bmy, 3/7/05)
!
!      FUNCTION E_ICE( TC ) 
!!
!!******************************************************************************
...
!  the dynamic timestep. (hyl, bey, bmy, djj, 4/2/99, 2/23/05)
!
...
! Prior to 2/23/05:
!  
! Phillipe Le Sager (of Nat'l Obs, Athens) originally noted that the following
! line of cde can cause precision problems on some compilers if WETLOSS and 
! GAINED are very close to each other, which will result in negative STT.
!
! Since:
!    WETLOSS = STT(I,J,L,N) * WASHFRAC - GAINED
!
! then this implies:
!    STT(I,J,L,N) = STT(I,J,L,N) - STT(I,J,L,N) * WASHFRAC + GAINED -  GAINED
!
! which reduces to:
!    STT(I,J,L,N) = STT(I,J,L,N) - STT(I,J,L,N) * WASHFRAC
!
! or:
!    STT(I,J,L,N) = STT(I,J,L,N) * ( 1d0 - WASHFRAC )
!
! So replace with this expression to avoid numerical blowup. (bmy, 2/23/05)
!
...
!$OMP+SCHEDULE( DYNAMIC )
      ...
      DO I = 1, IIPAR
         ...
         !--------------------------------------------------------------------
         ! Prior to 3/7/05:
         ! Now use new function E_ICE, which is a function of Kelvin
         ! temperature instead of Celsius (bmy, 3/7/05)
         !!=============================================================
