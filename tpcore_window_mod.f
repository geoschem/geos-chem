...
!------------------------------------------------------------------------------
! Prior to 3/12/03:
! Comment out MFCT code for now (bmy, 3/12/03)
!      if( MFCT ) then
! 
...
!#if   defined( CRAY       )
!CMIC$ do all
!CMIC$* shared(NL,im,jm,j1,jm1,qlow_w,DQ_w,delp2_w)
!CMIC$* private(i,j,k)
!#elif defined( SGI        )
!!$OMP PARALLEL DO PRIVATE( I, J, K )
...
!!%%% 
!!%%% MODIFICATIONS FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Now use QLOW_W array
!!%%%
!         DO J = 1, JM_W
!         DO I = 1, IM_W
!            QLOW_W(I,J,K) = DQ_W(I,J,K) / DELP2_W(I,J,K)
!         ENDDO
!         ENDDO
!!%%% 
!!%%% MODIFICATIONS FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Use extrapolation to compute qlow_w(0,j,k), qlow(im_w+1,j,k) 
!!%%%
!       DO J = 1, JM_W
!          QLOW_W(0,J,K)      = 2*QLOW_W(1,J,K)    - QLOW_W(2,J,K)
!          QLOW_W(IM_W+1,J,K) = 2*QLOW_W(IM_W,J,K) - QLOW_W(IM_W-1,J,K)
!       enddo
!!%%% 
!!%%% MODIFICATIONS FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Use extrapolation to compute qlow_w(i,j,k), qlow(i,jm_w+1,k) 
!!%%%
!       DO I = 1, IM_W
!          QLOW_W(I,0,K)      = 2*QLOW_W(I,1,K)    - QLOW_W(I,2,K)
!          QLOW_W(I,JM_W+1,K) = 2*QLOW_W(I,JM_W,K) - QLOW_W(I,JM_W-1,K)
!       enddo
!!%%% 
!!%%% MODIFICATIONS FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Polar cap stuff, useless for nested simulation
...
!!%%% 
!!%%% MODIFICATIONS FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Call FCT3D with window arrays, etc.
!!%%%
!!       call FCT3D(Q(1,1,1,IC),qlow_w,fx_w,fy_w,fz_w,IM_w,JM_w,NL,
!!     &   delp2_w,DPI_w,qz_w,wk_w,Qmax_w,Qmin_w,DG2_w,U_w,V_w,acosp_w,
!!     &   IM,JM,igzd)
...
! Comment out MFCT code for now (bmy, 3/12/03)
!
!      subroutine FCT3D(P,plow,fx,fy,fz,im,jm,km,delp,adx,ady,
!     &                 wk1,Qmax,Qmin,wkx,CRX,CRY,acosp,imm,jmm,igzd)
!C****6***0*********0*********0*********0*********0*********0**********72
!
!      ! Added to pass C-preprocessor switches (bmy, 3/9/01)
!#     include "define.h"
...
!      REAL*8 P(IMm,JMm,km),CRX(IM+2*igzd,JM+2*igzd,km),
!     &     p_tempt(im+2,jm+2,km),CRY(IM+2*igzd,JM+2*igzd,km),
!     &     Qmax(IM,JM,km),Qmin(IM,JM,km),acosp(*),delp(im,jm,km),
!     &     adx(IM,JM,km),ady(IM,JM,km),fx(IM+1,JM,km),
!     &     fy(IM,JM,km),fz(im,jm,km+1),wk1(IM+2*igzd,JM+2*igzd,km),
!     &     wkx(im+2*igzd,jm+2*igzd),wkn(im,jm),plow(IM+2,JM+2,km)
!C============================================================================
!C Cray NOBOUNDS directive will turn off all subscript bounds checking.
!C This directive is also legal on SGI compilers (bmy, 4/24/00)
!CDIR$ NOBOUNDS
...
! 
!      JM1 = JM-1
!C let P_tempt be the offsetted P
!      do j=0,jm+1
!      do i=0,im+1
!         p_tempt(i,j,:)=p(i+i0_w,j+j0_w,:)
!      enddo
!      enddo 
!
!C Find local min/max of the low-order monotone solution
!      call hilo3D(P_tempt,im,jm,km,adx,ady,Qmax,Qmin,wkx,wkn)
!      call hilo3D(plow,im,jm,km,Qmax,Qmin,wk1,P,wkx,wkn)
!C P is destroyed!
! 
!C     GOTO 123
!#if   defined( multitask  )
!#if   defined( CRAY       )
!CMIC$ do all autoscope
!CMIC$* shared(im,j1,km,CRX,CRY,adx,ady,Qmax,Qmin)
...
!      if(IT .lt. 1-igzd) then
!            IT = 1-igzd
!      elseif(IT .GT. IM+igzd) then
!            IT =IM+igzd 
!      endif
! 
...
!      enddo
! 
!C compute adx(0,j,k) and adx(im+1,j,k) by extrapolation (yxw,08/27/01)
!      do j=1,jm
!         adx(0,j,k)=2*adx(1,j,k)-adx(2,j,k)
!         adx(im+1,j,k)=2*adx(im,j,k)-adx(im-1,j,k)
!         ady(0,j,k)=2*ady(1,j,k)-ady(2,j,k)
!         ady(im+1,j,k)=2*ady(im,j,k)-ady(im-1,j,k) 
!      enddo
!
!C compute adx(0,j,k) and adx(im+1,j,k) by extrapolation (yxw,08/27/01) 
!      do i=1,im
!         adx(i,0,k)=2*adx(i,1,k)-adx(i,2,k)
!         adx(i,jm+1,k)=2*adx(i,jm,k)-adx(i,jm-1,k)
!         ady(i,0,k)=2*ady(i,1,k)-ady(i,2,k)
!         ady(i,jm+1,k)=2*ady(i,jm,k)-ady(i,jm-1,k)
!      enddo
!!%%%
!!%%% MODIFICATION FOR NESTED GRID (yxw, bmy, 3/10/03)
!!%%% Polar cap stuff, useless for nested grid 
!!%%% don't need the polar part (yxw,08/27/01)
!!%%%! S Pole
!!%%%      Ain = 0.
!!%%%      Aou = 0.
!!%%%      DO i=1,IM
