...
!   integer             ::  NPPequilibriumYear=50 !# of iteration years
!                             !  necessary to get soil moisture (and
!                             !  thus NPP and the decay scalars) in
!                             !  equilibrium.  3 is ok
!
!   integer             ::  HgPoolsequilibriumYear=30161 !# of iteration
!                             !  years necessary to get Hg pools in
!                             !  equilibrium.  ~30000 years with an
!                             !  f_decomp = 0.16, +161 to go from preind -> ind
!   integer             ::  preindYear=30000 !last preindustrial year of simulation
!   integer             ::  indYear=30161 ! last industrial year of simulation
                                         ! 161 years covering 1840 to 2000

!   integer             ::  NPPequilibriumYear=0  !# of iteration years
!                             !  necessary to get soil moisture (and
!                             !  thus NPP and the decay scalars) in
!                             !  equilibrium.  3 is ok
!
!   integer             ::  HgPoolsequilibriumYear=10  !# of iteration
!                             !  years necessary to get Hg pools in
!                             !  equilibrium.  ~30000 years with an
!                             !  f_decomp = 0.16, +161 to go from preind -> ind
!   integer             ::  preindYear=8 !last preindustrial year of simulation
!   integer             ::  indYear=10  ! last industrial year of simulation
                                         ! 161 years covering 1840 to 2000
   ...
   integer             ::  n_age_classes=1  !# of age classes, set to 1
			     !  GTMM DOES NOT FULLY SUPPORT AGE CLASSES
                             !  SO LEAVE = 1
...
!   integer             ::  f_len_output=27 !length of the outputpath
!
!   character(len=27)   ::  outputpath=&
!              '/home/ccarouge/GTMM/output/'
