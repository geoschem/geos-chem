      ...
      !calculates combustion completeness (CC, aka combustion
      ...
      !biomassCC
      fid(:,1)=0.0d0
      fid(:,1)=2.000d0*NPPmoist(:,mo)-1.00d0 !NPP moisture scalar runs from 
...
!      DO i=1, n_veg
!         IF (PET_current(i,1) .eq. 0d0) THEN 
!                 PET_current(i,1)=0.1000d0 !prevents dividing by 0
!         ENDIF
!      END DO
...
!--- Previous to (ccc, 11/10/09)
!      DO i=1, n_veg
!         IF (CCratio_current(i,1) .gt. 1d0) THEN
!                 CCratio_current(i,1)=1.000d0
!         ENDIF
!         CCratio_current(i,1)=1.000d0-CCratio_current(i,1)
!      END DO
