...
!      DO i=1,n_veg
!          IF (metabfract(i,1) .lt. 0d0) THEN
!                  metabfract(i,1) = 0.00d0
!          ENDIF
!      END DO
...
!                 soilmicDecayFactor(i,1)=soilmicDecayFactor(i,1)*1.25d0
!                 slowDecayFactor(i,1)=slowDecayFactor(i,1)*1.5d0
!                 armoredDecayFactor(i,1)=armoredDecayFactor(i,1)*1.5d0
!         END IF
!      END DO
      WHERE (veg1(:,1) == 12)
...
!--- Previous to (ccc, 11/12/09)
!      DO i=1,n_veg
!          IF (fid(i,1) .lt. 0d0) THEN
!                  decayClayFactor(i,1)=0.000d0
!          END IF
!          IF (soilmicDecayFactor(i,1) .gt. 1.0d0) THEN
!              soilmicDecayFactor(i,1)=1.0d0
!          ENDIF
!      END DO
      WHERE (fid(:,1) < 0d0)
