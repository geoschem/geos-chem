...
!$OMP WORKSHARE
      ...
      FPAR(:,mo)=(((sr(:,1)-SRMIN)/(srmax(:,1)-SRMIN))*(FPARMAX-FPARMIN))+FPARMIN
!$OMP END WORKSHARE
...
!$OMP DO PRIVATE(i)
      DO i=1, n_veg
         ...
         IF (veg1(i,1) .eq. 3) THEN
                 LAI_temp(i,1)=(LAImax(i,1)*(log(1d0-FPAR(i,mo))/log(1d0-FPARMAX)+FPAR(i,mo)/FPARMAX))/2d0
         ENDIF

         !homogeneous canopy

         IF (veg1(i,1) .ne. 3 .and. veg1(i,1) .ne. 4 .and. veg1(i,1) .ne. 5 .and. veg1(i,1) .ne. 9) THEN
