...
!      END DO
      ...
      hsurfstrpool(:,1)=hsurfstrpool(:,1)+carbonout_leaf(:,1)*(1.00d0-metabfract(:,1))
      ...
      carbonout_soilmic(:,1)=hsurfmicpool(:,1)*K_soilmic(:,1)*abiotic(:,mo)*soilmicDecayFactor(:,1)
      ...
      hsurfstrpool(:,1)=hsurfstrpool(:,1)+carbonout_leaf(:,1)*(1.00d0-metabfract(:,1))
      hsoilstrpool(:,1)=hsoilstrpool(:,1)+carbonout_froot(:,1)*(1.00d0-metabfract(:,1))
      hsurfmetpool(:,1)=hsurfmetpool(:,1)+carbonout_leaf(:,1)*metabfract(:,1)
      ...
      resppool_surfstr(:,1)=resppool_surfstr(:,1)+(temp(:,1)/eff_surfstr2slow)*(1.00d0-eff_surfstr2slow)

      temp(:,1)=0.0d0
      temp(:,1)=(carbonout_surfstr(:,1)*(1d0-structuralLignin(:,1)))*eff_surfstr2surfmic
      hsurfmicpool(:,1)=hsurfmicpool(:,1)+temp(:,1)
      resppool_surfstr(:,1)=resppool_surfstr(:,1)+(temp(:,1)/eff_surfstr2surfmic)*(1.000d0-eff_surfstr2surfmic)
      hsoilstrpool(:,1)=hsoilstrpool(:,1)-carbonout_soilstr(:,1)

      temp(:,1)=0.0d0
      temp(:,1)=carbonout_soilstr(:,1)*structuralLignin(:,1)*eff_soilstr2slow
      hslowpool(:,1)=hslowpool(:,1)+temp(:,1)
      resppool_soilstr(:,1)=resppool_soilstr(:,1)+(temp(:,1)/eff_soilstr2slow)*(1.000d0-eff_soilstr2slow)

      temp(:,1)=0.0d0
      temp(:,1)=carbonout_soilstr(:,1)*(1-structuralLignin(:,1))*eff_soilstr2soilmic
      hsurfmicpool(:,1)=hsurfmicpool(:,1)+temp(:,1)
