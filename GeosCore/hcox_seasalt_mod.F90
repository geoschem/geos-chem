...
!$OMP SCHEDULE( DYNAMIC )
      ...
      DO I = 1, HcoState%NX
         ...
         ! Do for accumulation and coarse mode
         DO N = 1,2
...
! !INTERFACE:
!
      SUBROUTINE HcoX_SeaSalt_Init ( am_I_Root, HcoState, ExtName, &
                                     ExtState,    RC                  ) 
!
! !USE:
!
...
!
      ...
      WRITE(MSG,*) 'Accumulation aerosol: ', &
                   TRIM(SpcNames(1)), ':', IDTSALA 
      CALL HCO_MSG(MSG)
      WRITE(MSG,*) ' - size range: ', SALA_REDGE_um
      CALL HCO_MSG(MSG)
      WRITE(MSG,*) 'Coarse aerosol: ', &
                   TRIM(SpcNames(2)), ':', IDTSALC
      CALL HCO_MSG(MSG)
      WRITE(MSG,*) ' - size range: ', SALA_REDGE_um
      CALL HCO_MSG(MSG)
