...
!
  ...
  PUBLIC :: IS_PASSIVE_TRACER
  PUBLIC :: PASSIVE_TRACER_INITCONC
...
!BOP
!
! !IROUTINE: IS_PASSIVE_TRACER
!
! !DESCRIPTION: Function IS\_PASSIVE\_TRACER returns TRUE if the given
!  GEOS-Chem tracer ID is a passive tracer, FALSE otherwise. 
!\\
!\\
! !INTERFACE:
!
  FUNCTION IS_PASSIVE_TRACER ( TRCID ) RESULT ( IsPassive )
!
! !USES:
!
!
...
! !OUTPUT PARAMETERS:
!
    LOGICAL                          :: IsPassive  ! Is passive tracer? 
!
! !REMARKS
!
! !REVISION HISTORY: 
...
!
  ...
  END FUNCTION IS_PASSIVE_TRACER
!EOC
!------------------------------------------------------------------------------
!                  Harvard-NASA Emissions Component (HEMCO)                   !
!------------------------------------------------------------------------------
!BOP
!
! !IROUTINE: PASSIVE_TRACER_INITCONC
!
! !DESCRIPTION: Function PASSIVE\_TRACER\_INITCONC returns the default initial
!  concentration in v/v of the given passive tracer (by GC tracer ID). 
!\\
!\\
! !INTERFACE:
!
  FUNCTION PASSIVE_TRACER_INITCONC ( TRCID ) RESULT ( InitConc )
!
! !USES:
!
!
...
! !OUTPUT PARAMETERS:
!
    REAL(fp)                         :: InitConc   ! Initial concentration (v/v) 
!
! !REMARKS
!
! !REVISION HISTORY: 
...
!
    ...
    !=================================================================
    ! PASSIVE_TRACER_INITCONC begins here!
    !=================================================================
    ...
    DO N = 1, NPASSIVE
       IF ( PASSIVE_ID(N) == TRCID ) THEN
          InitConc = PASSIVE_INITCONC(N)
          EXIT
       ENDIF
    ENDDO

  END FUNCTION PASSIVE_TRACER_INITCONC
!EOC
!------------------------------------------------------------------------------
!                  Harvard-NASA Emissions Component (HEMCO)                   !
!------------------------------------------------------------------------------
