...
150       CONTINUE
          ...
          DO 160  K = 1,NUMDEP
             ...
             ! If a POPs simulation, scale cuticular resistances with octanol-
             ! air partition coefficient (Koa) instead of HSTAR (clf, 1/3/2011)
             IF (IS_POPS) &
                  RLUXX = RLU(LDT)/(KOA(K)/1.0e+05_f8 + F0(K))

             !*
             !* To prevent virtually zero resistance to species with huge HSTAR,
             !* such as HNO3, a minimum value of RLUXX needs to be set. The
             !* rationality of the existence of such a minimum is demonstrated
             !* by the observed relationship between Vd(NOy-NOx) and Ustar in
             !* Munger et al.[1996];
...
!
    ...
    DO N = 1, State_Chm%nAdvect
       ...
       IF ( SpcInfo%Is_Drydep ) THEN
          ...
          CASE( 'DHDN', 'ETHLN', 'ISN1', 'MONITS', 'MONITU', 'HONIT' )
             ...
             ! for the computation of Vd to work properly.
             XMW(NUMDEP)  = State_Chm%SpcData(id_ISOPND)%Info%MW_g * 1e-3_fp
             FLAG(NUMDEP) = 3
