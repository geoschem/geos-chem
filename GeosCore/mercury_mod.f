...
#     include "CMN_SIZE"          ! Size parameters
      ...
      ! Choose whether to use CASA terrestrial Hg emissions from a file
      ! (generated by nvd) or whether to use mixed archived and online
      ! terrestrial emissions as described in Selin et al. 2008
      ! Eventually this switch can be set in input.geos
      LOGICAL, PARAMETER :: LCASA_HG = .FALSE.
      ...
      IF ( LCASA_HG .AND. ITS_A_NEW_MONTH() ) THEN

         ! NICOLE: Add a call to your subroutine here and 
         ! set LCASA_HG=.TRUE.  in the subroutine variable declarations.
         ! It might look like this:
         !
         ! THISMONTH = GET_MONTH()
         ! CALL READ_CASA_HG_EMISSION( THISMONTH )
         !
         ! READ_NASA_TRANSP might be a template for your subroutine
         ! Does CASA include the geogenic Hg emissions? If so then
         ! we need to turn them off here. Does CASA include biomass burning
         ! emissions? If not then CALL BIOMASSHG should go outside this
         ! IF statement (below).
         ! I think you can put the CASA emissions in any of these
         ! arrays: EHg0_vg, EHg0_ln, EHg0_so, EHg0_bb
