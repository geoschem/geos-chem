...
!BOP
!
! !ROUTINE: Config_ReadFile
!
! !DESCRIPTION: Subroutine Config_ReadFile reads the HEMCO configuration file
...
!
      ...
      DO
         ...
         ELSEIF ( DctType == 2 .OR. DctType == 3 ) THEN
      ...
      DO WHILE ( FLAG == HCO_SUCCESS ) 
         ...
         IF ( Lct%Dct%DctType == 1 ) THEN
            ...
            IF ( ExtNrInUse( Lct%Dct%ExtNr ) ) THEN
               ...
               ! verbose
               IF ( verb ) THEN
                  WRITE(MSG,*) 'Assigned HEMCO species ID: ', &
                                       Lct%Dct%HcoID
                  CALL HCO_MSG(MSG)
               ENDIF       
...
! !INTERFACE:
!
      SUBROUTINE Get_targetID ( am_I_Root, HcoState, &
                                Lct,       targetID, RC ) 
!
! !USES:
!
...
!BOP
!
! !ROUTINE: ConfigList_AddCont
!
! !DESCRIPTION: Subroutine ConfigList_AddCont adds a new (blank) container to
! the ConfigList list.
!\\
!\\
! !INTERFACE:
...
!BOP
!
! !ROUTINE: SpecName_Register 
!
! !DESCRIPTION: Subroutine SpecName_Register adds the species name SpecName 
...
!BOP
!
! !ROUTINE: SpecName_Cleanup
!
! !DESCRIPTION: Subroutine SpecName_Cleanup cleans up the internal SpecName
! list.
!\\
!\\
! !INTERFACE:
