...
#if   defined( APM )
      ! A-6 fields used by interpolation for APM simulation
      USE DAO_MOD,      ONLY : CLDF0,    CLDFRC0,   CLDMAS0,   CLDTOPS0 
      USE DAO_MOD,      ONLY : CMFMC0,   DETRAINE0, DETRAINN0, DNDE0    
      USE DAO_MOD,      ONLY : DNDN0,    DQIDTMST0, DQLDTMST0, DQRCON0
      USE DAO_MOD,      ONLY : DQRLSC0,  DQVDTMST0, DTRAIN0,   ENTRAIN0
      USE DAO_MOD,      ONLY : HKBETA0,  HKETA0    
      USE DAO_MOD,      ONLY : MOISTQ0,  OPTDEP0,   PV0
      USE DAO_MOD,      ONLY : QI0,      QL0,       RH0,       SPHU0
      USE DAO_MOD,      ONLY : T0,       TAUCLI0,   TAUCLW0,   UPDE0
      USE DAO_MOD,      ONLY : UPDN0,    UWND0,     VWND0,     ZMEU0
      USE DAO_MOD,      ONLY : ZMMD0,    ZMMU0

      USE DAO_MOD, ONLY : CLDF0a,    CLDFRC0a,   CLDMAS0a,   CLDTOPS0a
      USE DAO_MOD, ONLY : CMFMC0a,   DETRAINE0a, DETRAINN0a, DNDE0a
      USE DAO_MOD, ONLY : DNDN0a,    DQIDTMST0a, DQLDTMST0a, DQRCON0a
      USE DAO_MOD, ONLY : DQRLSC0a,  DQVDTMST0a, DTRAIN0a,   ENTRAIN0a
      USE DAO_MOD, ONLY : HKBETA0a,  HKETA0a
      USE DAO_MOD, ONLY : MOISTQ0a,  OPTDEP0a,   PV0a
      USE DAO_MOD, ONLY : QI0a,      QL0a,       RH0a,       SPHU0a
      USE DAO_MOD, ONLY : T0a,       TAUCLI0a,   TAUCLW0a,   UPDE0a
      USE DAO_MOD, ONLY : UPDN0a,    UWND0a,     VWND0a,     ZMEU0a
      USE DAO_MOD, ONLY : ZMMD0a,    ZMMU0a

      USE DAO_MOD, ONLY : CLDF0b,    CLDFRC0b,   CLDMAS0b,   CLDTOPS0b
      USE DAO_MOD, ONLY : CMFMC0b,   DETRAINE0b, DETRAINN0b, DNDE0b
      USE DAO_MOD, ONLY : DNDN0b,    DQIDTMST0b, DQLDTMST0b, DQRCON0b
      USE DAO_MOD, ONLY : DQRLSC0b,  DQVDTMST0b, DTRAIN0b,   ENTRAIN0b
      USE DAO_MOD, ONLY : HKBETA0b,  HKETA0b
      USE DAO_MOD, ONLY : MOISTQ0b,  OPTDEP0b,   PV0b
      USE DAO_MOD, ONLY : QI0b,      QL0b,       RH0b,       SPHU0b
      USE DAO_MOD, ONLY : T0b,       TAUCLI0b,   TAUCLW0b,   UPDE0b
      USE DAO_MOD, ONLY : UPDN0b,    UWND0b,     VWND0b,     ZMEU0b
      USE DAO_MOD, ONLY : ZMMD0b,    ZMMU0b
#endif
...
#     include "CMN_SIZE"  ! Size parameters
 ...
 100     FORMAT( '     - A-6 met fields for NYMD, NHMS = ', 
     ...
     &              CLDF=CLDF0,     CLDMAS=CLDMAS0, CLDTOPS=CLDTOPS0, 
     &              DTRAIN=DTRAIN0, MOISTQ=MOISTQ0, OPTDEPTH=OPTDEP0 )
#else
      CALL READ_A6( NYMD=NYMD,     NHMS=NHMS,       
     ...
     &              DTRAIN=DTRAIN, MOISTQ=MOISTQ, OPTDEPTH=OPTDEP )
      ...
      !=================================================================
#if   defined( APM )
      CALL READ_A6( NYMD=NYMD,      NHMS=NHMS,        CLDTOPS=CLDTOPS0,
     &              CLDF=CLDF0,     HKBETA=HKBETA0,   HKETA=HKETA0,   
     &              MOISTQ=MOISTQ0, OPTDEPTH=OPTDEP0, Q=SPHU0,        
     &              T=T0,           U=UWND0,          V=VWND0,        
     &              ZMEU=ZMEU0,     ZMMD=ZMMD0,       ZMMU=ZMMU0 )
#else
...
#if   defined( APM )
      CALL READ_A6( NYMD=NYMD,          NHMS=NHMS,       
     &              CLDF=CLDF0,         CLDTOPS=CLDTOPS0, 
     &              CMFMC=CMFMC0,       DQIDTMST=DQIDTMST0, 
     &              DQLDTMST=DQLDTMST0, DQRCON=DQRCON0, 
     &              DQRLSC=DQRLSC0,     DQVDTMST=DQVDTMST0,
     &              DTRAIN=DTRAIN0,     MOISTQ=MOISTQ0,
     &              OPTDEPTH=OPTDEP0, 
     &              PV=PV0,             RH=RH0,             
     &              Q=SPHU0,            QL=QL0,             
     &              QI=QI0,             T=T0,               
     &              TAUCLI=TAUCLI0,     TAUCLW=TAUCLW0,     
     &              U=UWND0,            V=VWND0         )
#else
      CALL READ_A6( NYMD=NYMD,          NHMS=NHMS,       
...
!----------------------------------------------------------------------------
     ...
     &              U=UWND,            V=VWND         ) 
      ...
      CALL READ_A6( NYMD=NYMD,          NHMS=NHMS,         
     &              CLDF=CLDF0,         CLDTOPS=CLDTOPS0, 
     &              DETRAINE=DETRAINE0, DETRAINN=DETRAINN0, 
     &              DNDE=DNDE0,         DNDN=DNDN0,         
     &              ENTRAIN=ENTRAIN0,   MOISTQ=MOISTQ0,
     &              OPTDEPTH=OPTDEP0,   Q=SPHU0,            
     &              T=T0,               U=UWND0,            
     &              UPDE=UPDE0,         UPDN=UPDN0,  
     &              V=VWND0 )
          
      ! Create 2-D CLDFRC field from 3-D CLDF field
      CALL MAKE_GCAP_CLDFRC( CLDF0, CLDFRC0 )
     ...
     &              V=VWND )
      ...
      IF ( FIRST )THEN
         ...
         CLDTOPS0b  = CLDTOPS0
         CMFMC0b    = CMFMC0
         DETRAINE0b = DETRAINE0
         DETRAINN0b = DETRAINN0
         DNDE0b     = DNDE0
         DNDN0b     = DNDN0
         DQIDTMST0b = DQIDTMST0
         DQLDTMST0b = DQLDTMST0
         DQRCON0b   = DQRCON0
         DQRLSC0b   = DQRLSC0
         DQVDTMST0b = DQVDTMST0
         DTRAIN0b   = DTRAIN0
         ENTRAIN0b  = ENTRAIN0
         HKBETA0b   = HKBETA0
         HKETA0b    = HKETA0
      ...
      CLDTOPS0a     = CLDTOPS0b
      CMFMC0a       = CMFMC0b
      DETRAINE0a    = DETRAINE0b
      DETRAINN0a    = DETRAINN0b
      DNDE0a        = DNDE0b
      DNDN0a        = DNDN0b
      DQIDTMST0a    = DQIDTMST0b
      DQLDTMST0a    = DQLDTMST0b
      DQRCON0a      = DQRCON0b
      DQRLSC0a      = DQRLSC0b
      DQVDTMST0a    = DQVDTMST0b
      DTRAIN0a      = DTRAIN0b
      ENTRAIN0a     = ENTRAIN0b
      HKBETA0a      = HKBETA0b
      HKETA0a       = HKETA0b
...
!******************************************************************************
      ...
      USE DAO_MOD,      ONLY : DNDN,    DQIDTMST, DQLDTMST, DQRCON
      USE DAO_MOD,      ONLY : DQRLSC,  DQVDTMST, DTRAIN,   ENTRAIN
      USE DAO_MOD,      ONLY : HKBETA,  HKETA    
      USE DAO_MOD,      ONLY : MOISTQ,  OPTDEP,   PV
      USE DAO_MOD,      ONLY : QI,      QL,       RH,       SPHU
      USE DAO_MOD,      ONLY : T,       TAUCLI,   TAUCLW,   UPDE
      USE DAO_MOD,      ONLY : UPDN,    UWND,     VWND,     ZMEU
      USE DAO_MOD,      ONLY : ZMMD,    ZMMU
      ...
      ! Return to calling program
      END SUBROUTINE UPDATE_A6_FIELDS
