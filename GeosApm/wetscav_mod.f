...
#if   defined( APM )
      REAL*8, PUBLIC, ALLOCATABLE :: SO2GAINED(:,:,:)
      REAL*8, PUBLIC, ALLOCATABLE :: SO2WETLOSS(:,:,:)
#endif
!
...
!  25 Mar 2011 - R. Yantosca - Need to increase NSOLMAX for APM microphysics
!EOP
!------------------------------------------------------------------------------
!BOC
!
...
!
      !-----------------------------------------------------------------------
      ! Prior to 3/25/11:
      ! Need to increase NSOLMAX for APM microphysics (bmy, 3/25/11)
      !INTEGER, PARAMETER   :: NSOLMAX = 50      ! Max # of soluble tracers
      !-----------------------------------------------------------------------
      INTEGER, PARAMETER   :: NSOLMAX = 100     ! Max # of soluble tracers
...
#if   defined( APM )
      !---------------------------------------
      ! %%% TRACERS FOR APM MICROPHYSICS %%%
      !---------------------------------------

      ELSE IF ( N >= IDTSO4G .and. N<(IDTSO4G+NGCOND)) THEN
         CALL F_AEROSOL( KC, F )
...
!
      ...
      USE OCEAN_MERCURY_MOD,  ONLY : LHg_WETDasHNO3
#if   defined( APM )
      USE APM_INIT_MOD, ONLY : NDSTB,IDTDSTBIN1
#endif
...
!         IF ( TK < 248d0 ) RAINFRAC = 0d0
      ...
      ELSE IF ( N >= IDTDSTBIN1 .and. N < (IDTDSTBIN1+NDSTB) ) THEN
         RAINFRAC = GET_RAINFRAC( K_RAIN, F, DT )
...
#if   defined( APM )
            ! Archive GAINED for APM microphysics
            SO2GAINED(I,J,L) = GAINED * 96D0 / 64D0
#endif
...
#if   defined( APM )
            ! Archive WETLOSS for APM microphysics
            SO2WETLOSS(I,J,L) = WETLOSS * 96d0 / 64d0
#endif
...
! !LOCAL VARIABLES:
!
      INTEGER :: N, NN, N_TOT_TRC, N_TOT_SOL
...
#if   defined( APM )
      N_TOT_TRC = N_TRACERS + NDSTB   ! G-C tracers + APM dust tracers
      N_TOT_SOL = NSOL      - NDSTB   ! G-C soluble tracers only
#else 
      N_TOT_TRC = N_TRACERS           ! G-C tracers only
      N_TOT_SOL = NSOL                ! G-C soluble tracers only
#endif
...
#if   defined( APM )
         ELSE IF ( N > N_TRACERS ) THEN
            NSOL         = NSOL + 1
            IDWETD(NSOL) = IDTDSTBIN1 + N - N_TRACERS - 1
#endif
...
#if   defined( APM )
         IF ( LDUST  )   NMAX = NMAX + NDSTB    ! plus # of apm dust bins
#endif
...
#if   defined( APM )
         ! Allocate SO2GAINED for APM
         ALLOCATE( SO2GAINED( IIPAR, JJPAR, LLPAR ), STAT=AS )
         IF ( AS /= 0 ) CALL ALLOC_ERR( 'SO2GAINED' )
         SO2GAINED = 0d0

         ! Allocate SO2WETLOSS for APM
         ALLOCATE( SO2WETLOSS( IIPAR, JJPAR, LLPAR ), STAT=AS )
         IF ( AS /= 0 ) CALL ALLOC_ERR( 'SO2WETLOSS' )
         SO2WETLOSS = 0d0
#endif
...
#if   defined( APM )
      IF ( ALLOCATED( SO2GAINED  ) ) DEALLOCATE( SO2GAINED )
      IF ( ALLOCATED( SO2WETLOSS ) ) DEALLOCATE( SO2WETLOSS )
#endif
