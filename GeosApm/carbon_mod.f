...
#if   defined( APM )
      IF ( NBCPOF > 0 ) THEN
         CALL CHEM_BCPONEW( STT(:,:,:,IDTBCPOFF) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_BCPO' )
      ENDIF

      IF ( NBCPIF >0 )THEN
         CALL CHEM_BCPINEW( STT(:,:,:,IDTBCPIFF) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_BCPI' )
      ENDIF

      IF ( NBCPOO >0 )THEN
         CALL CHEM_BCPONEW( STT(:,:,:,IDTBCPOBB) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_BCPO' )
      ENDIF

      IF ( NBCPIO >0 )THEN
         CALL CHEM_BCPINEW( STT(:,:,:,IDTBCPIBB) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_BCPI' )
      ENDIF
#endif
...
#if   defined( APM )
      IF ( NOCPOF > 0 ) THEN
         CALL CHEM_OCPONEW( STT(:,:,:,IDTOCPOFF) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_OCPO' )
      ENDIF

      IF ( NOCPIF >0 )THEN
         CALL CHEM_OCPINEW( STT(:,:,:,IDTOCPIFF) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_OCPI' )
      ENDIF

      IF ( NOCPOO >0 )THEN
         CALL CHEM_OCPONEW( STT(:,:,:,IDTOCPOBB) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_OCPO' )
      ENDIF

      IF ( NOCPIO >0 )THEN
         CALL CHEM_OCPINEW( STT(:,:,:,IDTOCPIBB) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBON: a CHEM_OCPI' )
      ENDIF
#endif
...
#if   defined( APM )
      SUBROUTINE CHEM_BCPONEW( TC )
! Modified for APM simulation
!******************************************************************************
!  Subroutine CHEM_BCPONEW converts hydrophobic BC to hydrophilic BC and
!  calculates the dry deposition of hydrophobic BC. (rjp, bmy, 4/1/04,10/25/05)
!
!  Arguments as Input:
!  ============================================================================
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CHEM_BCPINEW( TC )
! Modified for APM simulation
!******************************************************************************
!  Subroutine CHEM_BCPINEW calculates dry deposition of hydrophilic BC.
!  (rjp, bmy, 4/1/04, 10/25/05)
!
!  Arguments as Input:
!  ============================================================================
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CHEM_OCPONEW( TC )
! Modified for APM simulation
!******************************************************************************
!  Subroutine CHEM_OCPONEW converts hydrophobic OC to hydrophilic OC and
!  calculates the dry deposition of hydrophobic OC. (rjp, bmy, 4/1/04,10/25/05)
!
!  Arguments as Input:
!  ============================================================================
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CHEM_OCPINEW( TC )
! Modified for APM simulation
!******************************************************************************
!  Subroutine CHEM_OCPINEW calculates dry deposition of hydrophilic OC.
!  (rjp, bmy, 4/1/04, 10/25/05)
!
!  Arguments as Input:
!  ============================================================================
...
!------------------------------------------------------------------------------
#if      defined( APM )
         CALL CHEM_NVOC( I, J, L, RTEMP,FAC,KO3, KOH, KNO3, GM0 )
#else
         CALL CHEM_NVOC( I, J, L, KO3, KOH, KNO3, GM0 )
#endif
...
!     
      ...
      ! OX = OH 
!#if   defined( APM )
!      !APM_Yu: To make it consistent with other VOCs, switch the position to  
!      !         put lower vapor pressure one as the first product
!      ALPHA(1,1,6) = 0.0288d0   
!      ALPHA(1,2,6) = 0.232d0 
!#else
      ALPHA(1,1,6) = 0.232d0
...
#if   defined( APM )
      SUBROUTINE CHEM_NVOC(I,J,L,RTEMP,FAC,KO3, KOH, KNO3, GM0 )
#else
...
!
      ...
      !USE TRACERID_MOD
#if   defined( APM )
      USE DIAG_MOD,     ONLY : AD07, AD07_OM
      USE DAO_MOD,      ONLY : PBL, IS_WATER
      USE GRID_MOD,     ONLY : GET_AREA_M2
      USE ERROR_MOD,    ONLY : DEBUG_MSG
      USE LOGICAL_MOD,  ONLY : LSOA, LPRT, LCOOKE
      USE SULFATE_MOD,  ONLY : SSTEMP
      USE TIME_MOD,     ONLY : GET_YEAR, GET_MONTH, GET_TS_EMIS
      USE TIME_MOD,     ONLY : ITS_A_NEW_MONTH
      USE TRACER_MOD,   ONLY : STT
      USE APM_INIT_MOD, ONLY : ISITES,JSITES,IFSITE,IFEMITH
      USE APM_INIT_MOD, ONLY : IDTBCPIFF,IDTBCPIBB,IDTOCPIFF,IDTOCPIBB
      USE APM_INIT_MOD, ONLY : IDTBCPOFF,IDTBCPOBB,IDTOCPOFF,IDTOCPOBB
      USE TRACERID_MOD, ONLY : IDTISOP,IDTALPH
...
#if   defined( APM )
      INTEGER, SAVE        :: AOWD
      REAL*8,SAVE          :: OCMASS0(IIPAR,JJPAR)
      INTEGER              :: YEAR
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      ! Arguments
      REAL*8, INTENT(IN) :: BCSRCF(IIPAR,JJPAR,2)
      REAL*8, INTENT(IN) :: BCSRCB(IIPAR,JJPAR,2)
      REAL*8, INTENT(IN) :: BCSRCO(IIPAR,JJPAR,2)
      REAL*8, INTENT(IN) :: OCSRCF(IIPAR,JJPAR,2)
      REAL*8, INTENT(IN) :: OCSRCB(IIPAR,JJPAR,2)
      REAL*8, INTENT(IN) :: OCSRCO(IIPAR,JJPAR,2)
