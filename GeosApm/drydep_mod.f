...
#if   defined( APM )
      ! size resolved dry velocities for APM simulation
      REAL*8            :: DVELSIZE(MAXIJ,NSO4+NSEA+NDSTB)
#endif
...
!APM_GanLuo+
      ...
      !=================================================================
      ! Call DEPVELBIN to compute size resolved dry velocities
      ! for APM simulation [m/s]
      !=================================================================
      !GanLuo for newbins
      CALL DEPVELBIN( MAXIJ, RADIAT, TC0, SUNCOS,
     &                USTAR, CZ1,    OBK, CFRAC,  
     &                ZH,    LSNOW,  DVELSIZE,   AZO,    RHB )
...
!$OMP+PRIVATE( I, J, IJLOOP, THIK, N, NN, NDVZ, DVZ )
      ...
      DO I = 1, IIPAR
         ...
         ! Loop over drydep species
         DO N = 1, NSO4+NSEA+NDSTB

            ! Dry deposition velocity [cm/s]
            DVZ  = DVELSIZE(IJLOOP,N) * 100.d0

            ! Set minimum velocity for sulfate tracers
            DVZ  = MAX( 1.0d-2, DVZ )

            ! Dry deposition frequency [1/s]
            DEPSAVSIZE(I,J,N) = ( DVZ / 100.d0 ) / THIK
...
!$OMP END PARALLEL DO
      ...
      !### Debug
      IF ( LPRT ) CALL DEBUG_MSG( '### DO_DRYDEP: after DEPVELBIN' )
#endif
...
!------------------------------------------------------------------------------
#if   defined( APM )
      ! Compute size resolved dry velocities for APM simulation
      SUBROUTINE DEPVELBIN( NPTS, RADIAT, TEMP, SUNCOS,
     &                      USTAR,  CZ1,  OBK,  CFRAC,
     &                      ZH,   LSNOW,  DVEL, ZO,     RHB )

      ! Modified from GEOS-Chem SUBROUTINE DEPVEL
      USE ERROR_MOD,  ONLY : IT_IS_NAN
      USE APM_INIT_MOD, ONLY : NSO4,NSEA,NDSTB
...
C***********************************************************************       
C
C Need as landtype input for each grid square (I,J) see (RDLAND &
C CMN_VEL):
C     IJREG(JLOOP)       - # of landtypes in grid square
C     IJLAND(IJLOOP,LDT) - Land type ID for element LDT =1,
C     IJREG(IJLOOP)
C                          (could be from any source - mapped to
C                          deposition 
C                          surface ID in input unit 65)
C     IJUSE(IJLOOP,LDT)  - Fraction ((per mil) of gridbox area occupied
C     by
C                          land type element LDT
C
C Need as leaf area index see (RDLAI & CMN_VEL):
...
C
C Need as input for each species K (passed):
C     F0(K)          - reactivity factor for oxidation of biological
C     substances
C     HSTAR(K)       - Henry's Law constant
C     XMW(K)         - Molecular weight (kg/mole) of species K
C                      (used to calculate molecular diffusivities)
C
C Some variables used in the subroutine (passed):
C     LRGERA(IJLOOP) T -> stable atmosphere; a high aerodynamic
C     resistance
C                        (RA=1.E4 m s-1) is imposed; else RA is
C                        calculated
C     USTAR(IJLOOP)  - Friction velocity (m s-1)
C     CZ1(IJLOOP)    - Altitude (m) at which deposition velocity is
C     computed
C     OBK(IJLOOP)    - Monin-Obukhov length (m): set to 1.E5 m under
C     neutral 
C                      conditions
C     CFRAC(IJLOOP)  - Fractional cloud cover
C     ZH(IJLOOP)     - Mixing depth (m)
...
C     MAXDEP         - the maximum number of species for which the dry 
C                      deposition calculation is done
C     ZO(LDT)        - Roughness height (m) for specific surface type
C     indexed 
C                      by LDT
C     RSURFC(K,LDT)  - Bulk surface resistance (s m-1) for species K to 
C                      surface LDT
C     C1X(K)         - Total resistance to deposition (s m-1) for
C     species K
C
C Returned:
C     DVEL(IJLOOP,K) - Deposition velocity (m s-1) of species K
...
#     include "commsoil.h"
      ...
      REAL*8  USTAR(MAXIJ),CZ1(MAXIJ)
      REAL*8  OBK(MAXIJ),CFRAC(MAXIJ),ZH(MAXIJ)
      REAL*8  DVEL(MAXIJ,NSO4+NSEA+NDSTB)
      ...
      REAL*8  RI(NTYPE),RLU(NTYPE),RAC(NTYPE),RGSS(NTYPE),
     1        RGSO(NTYPE),RCLS(NTYPE),RCLO(NTYPE),
     2        RSURFC(NSO4+NSEA+NDSTB,NTYPE)       
                                   
      REAL*8  C1X(NSO4+NSEA+NDSTB),VD(NSO4+NSEA+NDSTB),
     &        VK(NSO4+NSEA+NDSTB)
...
C****************************************************************************
         DO 175 LDT = 1,IJREG(IJLOOP)

            IF (IJUSE(IJLOOP,LDT) .EQ. 0) GOTO 175
            IOLSON = IJLAND(IJLOOP,LDT)+1
            II = IDEP(IOLSON)
C
            IF(LSNOW(IJLOOP)) II=1
...
C*
            DO K = 1, NSO4
               ...
               ! [Zhang et al., 2001]
               RSURFC(K,LDT) = 
     &         PWET_SFCRSII( II, PRESS*1D-3, TEMPK, USTAR(IJLOOP),
     &         DENWET3D(I,J,1,1), GFTOT3D(I,J,1,1), RDRY(K))  
            ENDDO
 170        CONTINUE
...
!luogan add new dust
            DO K = 1, NDSTB
               ! [Zhang et al., 2001]
               RSURFC((NSO4+NSEA+K),LDT) =
     &         PWET_SFCRSII( II, PRESS*1D-3, TEMPK, USTAR(IJLOOP),
     &         DENDST(K), 1.d0, RDST(K))

            ENDDO

!luogan add new seasalt
            DO K = 1, NSEA
               ! [Zhang et al., 2001]
               RSURFC((NSO4+K),LDT) =
     &         PWET_SFCRSII( II, PRESS*1D-3, TEMPK, USTAR(IJLOOP),
     &         DENWET3D(I,J,1,2), GFTOT3D(I,J,1,2), RSALT(K))
            ENDDO
 175        CONTINUE
C*
C*    Set max and min values for bulk surface resistances         
C*                                 
         DO 190 K = 1,NSO4+NSEA+NDSTB
            DO 180 LDT = 1,IJREG(IJLOOP)
               IF (IJUSE(IJLOOP,LDT) .EQ. 0) GOTO 180
               RSURFC(K,LDT)= MAX(1.D0, MIN(RSURFC(K,LDT), 9999.D0))
 180        CONTINUE
 190     CONTINUE
C*                                 
C*    Loop through the different landuse types present in the grid
Csquare     
C*                                 
         DO 500 LDT=1, IJREG(IJLOOP)
            IF (IJUSE(IJLOOP,LDT) .EQ. 0) GOTO 500
...
C*********************************************************************
            ...
            ! Define REYNO for GCAP or GEOS met fields (swu, bmy,
            ! 5/25/05)
#if   defined( GCAP )
            REYNO = USTAR(IJLOOP)*ZO(LDT)/XNU 
#else
 ...
 211        FORMAT(1X,'OBK(IJLOOP)=',E11.2,1X,' IJLOOP = ',I4,1X,
     c           'LDT=',I3/) 
            ...
            ! Define Z0OBK for GCAP or GEOS met fields (swu, bmy,
            ! 5/25/05)
#if   defined( GCAP )
            Z0OBK = ZO(LDT)/OBK(IJLOOP)
#else
...
C*... unstable conditions; compute Ra as described above.
            ...
            ELSEIF((CORR1.GT.0.0D0).AND.(.NOT.LRGERA(IJLOOP)))THEN
C*...moderately stable conditions (z/zMO <1); compute Ra as described
Cabove
               RA = (1D0/CKUSTR) * 
     &              (.74D0*LOG(CORR1/Z0OBK) + 4.7D0*(CORR1-Z0OBK))
            ELSEIF(LRGERA(IJLOOP)) THEN
...
C* happened in version 3.1) send a warning message.
 ...
 1001       FORMAT('WARNING: RA < 0 IN SUBROUTINE DEPVEL',
     &             I10,4(1X,E12.5))
C* Get total resistance for deposition - loop over species.
            DO 215 K = 1,NSO4+NSEA+NDSTB
               C1X(K) = RA + RSURFC(K,LDT)
 215        CONTINUE
            GOTO 240
 220        CONTINUE 
C** ... aerodynamically smooth surface
C** BUG FIX -- suppress drydep over smooth surfaces by setting Ra to a
Clarge
C** value (1e4).  This prevents negative dry deposition velocities when
Cu*
C** is very small (djj, bmy, 5/8/00)
            DO 230 K = 1,NSO4+NSEA+NDSTB
                  RA     = 1.0D4
                  C1X(K) = RA + RSURFC(K,LDT)
 230        CONTINUE
...
C**********************************************************************
C** Load array DVELBIN
         DO 550 K=1,NSO4+NSEA+NDSTB
            DVEL(IJLOOP,K) = VD(K)
...
!$OMP END PARALLEL DO
      ...
      ! Return to calling program
      END SUBROUTINE DEPVELBIN

      FUNCTION PWET_SFCRSII( II, PRESS, TEMP, USTAR,
     &                       AERODEN, HGROW, A_RADIDRY) RESULT(RS)
!
!******************************************************************************
!
      ! Arguments
      INTEGER, INTENT(IN) :: II         ! Surface type index of GEOS-CHEM
      REAL*8,  INTENT(IN) :: PRESS      ! Pressure in Kpa 1 mb = 100 pa = 0.1kPa
      REAL*8,  INTENT(IN) :: TEMP       ! Temperature (K)    
      REAL*8,  INTENT(IN) :: USTAR      ! Friction velocity (m/s)
      REAL*8,  INTENT(IN) :: AERODEN    ! Relative humidity (fraction)
      REAL*8,  INTENT(IN) :: HGROW      ! Relative humidity (fraction)
      REAL*8,  INTENT(IN) :: A_RADIDRY  ! Sea salt radius (m)
     ...
     &           10.0d0, -999.d0, -999.d0, -999.d0, 10.0d0  /
      ...
      ! Aerosol growth with relative humidity in radius [m] 
      RWET = A_RADIDRY * HGROW 

      ! Diameter of the wet aerosol [m]
      DIAM  = RWET * 2.d0  

      ! Density of the wet aerosol [kg/m3]
      DEN   = AERODEN *1000.d0 !luo change g/cm3 to kg/m3 
...
#if   defined( APM )
      ALLOCATE( DEPSAVSIZE( IIPAR, JJPAR, NSO4+NSEA+NDSTB ), STAT=AS )
      IF ( AS /= 0 ) CALL ALLOC_ERR( 'DEPSAVSIZE' )
      DEPSAVSIZE = 0d0
#endif
...
#if   defined( APM )
      IF ( ALLOCATED( DEPSAVSIZE  ) ) DEALLOCATE( DEPSAVSIZE  )
#endif
