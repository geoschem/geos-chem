...
!
! !REVISION HISTORY
! 8 JULY 2011 - M. Long - Altered, Commented-out or Added Lines Tagged (v0.1)
!
!\\
!\\
! !INTERFACE:
...
!v0.1  USE GC_Column_Mod                                  ! GEOS-Chem column code
  USE GC_esmf_ChunkMod                                   ! GEOS-Chem Chunking code
!v0.1  USE Schem_Mod, ONLY : SCOX_1D                      ! Typedef for strat Ox

  IMPLICIT NONE
  PRIVATE

# include "smv_dimension.h"                          ! Max levels & tracers
# include "CMN_SIZE" !v0.1
# include "CMN_DIAG" !v0.1
# include "smv_errcode.h"                            ! GEOS-Chem error codes
# include "smv_physconst.h"                          ! Physical constants
!v0.1 # include "lai_land_info.h"                          ! Sizes for land/LAI data
!
...
!v0.1    ASSERT_( N_OLSON_LOCAL      == 15          )   ! # of lai/leaf types/box
...
!  
    ...
    INTEGER                      :: iReg_1d
    INTEGER                      :: iLand_1d   ( NTYPE           ) !v0.1
    INTEGER                      :: iUse_1d    ( NTYPE           ) !v0.1
    REAL*8                       :: lai_1d     ( NTYPE           ) !v0.1
...
!#      include "Includes_Before_Run.h"
       ...
       CALL GC_Chunk_Run                                              
!v0.1                         ( COEF          = Coef,                      &
!v0.1                           DIMINFO       = DimInfo,                   &
!v0.1                           GEOLOC        = GeoLoc,                    &
!v0.1                           ID_DRYDEP     = IdDrydep,                  &
!v0.1                           ID_EMISSION   = idEmission,                &
!v0.1                           ID_SPECIES    = IdSpecies,                 &
!v0.1                           ID_TRACERS    = IdTracers,                 &
!v0.1                           ID_WETDEP     = IdWetdep,                  &
!v0.1                           IDENT         = Ident,                     &
!v0.1                           MET_1d        = Met,                       &
!v0.1                           OPTIONS       = Options,                   &
!v0.1                           TIMING        = Timing,                    &
!v0.1                           ALK_EMIS_1d   = alkEmis_1d,                &
!v0.1                           N_DENS_1d     = nDens_1d,                  &
!v0.1                           IREG_1d       = iReg_1d,                   &
!v0.1                           ILAND_1d      = iLand_1d,                  &
!v0.1                           IUSE_1d       = iUse_1d,                   &
!v0.1                           LAI_1d        = lai_1d,                    &
!v0.1                           ORVC_TERP     = terp_1d,                   & 
!v0.1                           ORVC_SESQ     = sesq_1d,                   & 
!v0.1                           GPROD         = gProd_1d,                  &
!v0.1                           APROD         = aProd_1d,                  &
!v0.1                           OXIDANTS_1d   = Schem,                     &
!v0.1                           CSPEC_FULL_1d = cspec_1d,                  &
!v0.1                           H2O2s_1d      = h2o2s_1d,                  & 
!v0.1                           SO2s_1d       = so2s_1d,                   &
!v0.1                           EMISSION_1d   = emiss_1d   ( :, 1:N_TRC ), &
!v0.1                           TRACER_1d     = tracer_1d  ( :, 1:N_TRC ), &
!v0.1                           DD_VEL_1d     = ddVelDiag  (    1:N_TRC ), &
!v0.1                           DD_FREQ_1d    = ddFreqDiag (    1:N_TRC ), &
!v0.1                           DD_FLUX_1d    = ddFluxDiag (    1:N_TRC ), &
!v0.1                           WETD_LOSS_1d  = wdLossDiag ( :, 1:N_TRC ), &
!v0.1                           AIR_MASS_1d   = airMassDiag,               & 
!v0.1                           OH_MASS_1d    = ohMassDiag,                & 
!v0.1                           RC            = error )
!v0.1
       ! Trap the error from GEOS-Chem
       IF ( error /= SMV_SUCCESS ) THEN 
...
#   include "GEOSCHEMchem_GetPointer___.h"
    ...
    CALL GC_Chunk_Final( Ident, Coef, error )
...
! 
    ...
    TYPE(GEOSCHEM_State), POINTER :: myState        ! Legacy state
    TYPE(GEOSCHEM_Wrap)           :: wrap           ! Wrapper for myState
    TYPE(MAPL_MetaComp),  POINTER :: ggState        ! MAPL MetaComp object
    ...
    ! Lat & Longitude values on this PET
    IF ( PRESENT( lonCtr ) ) THEN
       CALL MAPL_Get( ggState, lons=lonCtr, &
                               lats=latCtr, &
                               __RC__ )
    ENDIF
    ...
    IF ( PRESENT( latEdg ) ) THEN
       CALL MAPL_Get( ggState, lats=latEdg, __RC__ )
    ENDIF 
...
! !INPUT PARAMETERS:
!
    REAL*4,            INTENT(IN)    :: lonRad   ! Longitude [radians]
    REAL*4,            INTENT(IN)    :: latRad   ! Latitude  [radians]
...
!  14 Apr 2010 - R. Yantosca - Adapted for use in GC_ESMF_COMP.F90
