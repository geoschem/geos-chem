...
CONTAINS
  ...
  !=========================================================================
  ! Hetchem rate-law functions for HO2  !!!yumin 20241007 phase state effect
  !=========================================================================
  ...
  FUNCTION HO2uptk1stOrd( H ) RESULT( k )
    ...
    REAL(dp)                   :: sgamma         ! local vars shell gamma
    REAL(dp)                   :: Horg           ! Henry's law constant for mixing OA surface in mol/l/atm
    REAL(dp)                   :: gamma          ! local vars total gamma for OA species
    !
    k    = 0.0_dp
    srMw = SR_MW(ind_HO2)
    !
    ...
    !--------------------------------------------------------------------------
    ! for organic aerosol surface only      
    Horg = 2.0e+6_dp   ! assuming from zhang et al., 2018

    sgamma = ShellGamma(srMw, 7, Horg, ORC, H ) ! use the mixing OA viscosity
    IF (sgamma > 0.0) THEN
    gamma = 1.0_dp/ (1.0_dp/H%gamma_HO2 + 1.0_dp/sgamma)
    ENDIF
    k = k + Ars_L1k( H%xArea(ORC), H%xRadi(ORC), gamma, srMw )
    ! finish the  OA surface
    !--------------------------------------------------------------------------
  ...
  FUNCTION NO2uptk1stOrdAndCloud( H ) RESULT( k )
    ...
    gamma = 1e-6_dp
    !------------------------------------------------------------------------
    ! yumin phase state effect from here 20241007
    sgamma = ShellGamma(srMw, 7, Horg, ORC, H ) ! use the mixing OA viscosity
    IF (sgamma > 0.0) THEN
    gamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgamma)
    ENDIF  
    !------------------------------------------------------------------------
    k = k + Ars_L1k( H%xArea(ORC), H%xRadi(ORC), gamma, srMw )
    !
  ...
  FUNCTION GLYXuptk1stOrd( srMw, H ) RESULT( k )
    ...
    REAL(dp)                   :: gamma  ! local vars
    REAL(dp)                   :: sgamma  ! local vars shell gamma
    REAL(dp)                   :: Horg    ! Henry's law constant for GLYX in mol/l/atm    

   !------------------------------------------------------------------------------
   !Caculate the Henry's law constants by consider the salt-in and salt-out effect
   !------------------------------------------------------------------------------
      REAL(dp)              :: KHw                ! T depended Henry constant for GLYX in pure water
      REAL(dp),  PARAMETER  :: HG  = 4.15e+5_dp   ! [M atm-1]
      REAL(dp),  PARAMETER  :: DhG = 7200e+0_dp   ! [K]
      ! salting constant for glyoxal [m-1]
      REAL(dp),  PARAMETER  :: KsCl   = -0.10_dp
      REAL(dp),  PARAMETER  :: KsSO4  = -0.24_dp
      REAL(dp),  PARAMETER  :: KsNIT  = -0.07_dp
      !---------------------------------------
    ...
    ! Henry's constant for aerosol [M atm-1]
     KHw= HG*exp(DhG*((1.e+0_dp/TEMP)-(1.e+0_dp/298.e+0_dp)))

     Horg = KHw ! consider the salt in and out effec later!!!!!!
       ...
       IF ( SUNCOS > 0.0_dp ) THEN
          ...
          ! yumin
          sgamma = ShellGamma(srMw, 5, Horg, SUL, H )

          IF (sgamma > 0.0_dp .AND. gamma > 0.0_dp) THEN
              gamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgamma)
  ...
  FUNCTION EpoxUptkGamma( srMw, H ) RESULT( gamma )
    ...
    !
    ! Calculate the second uptae parameterization term:
    val2 = ( 1.0_dp / MACOEFF )
    !
...
! Yumin Li 22/07/2024
  FUNCTION ShellGamma(srMw, visN, Horg, aerosol, H ) RESULT( gamma )
  ...
  ! Γ𝑜𝑟𝑔 = (4𝑅𝑇𝐻𝑜𝑟𝑔𝐷𝑜𝑟𝑔)/(𝜔𝑙𝑜𝑟𝑔)*𝑅𝑐/𝑅𝑝 
  ! 𝐷𝑜𝑟𝑔=  (𝑘𝐵 𝑇)/6𝜋𝜂𝑅𝑝 

    REAL(dp),       INTENT(IN) :: srMw           ! sqrt(mol wt)
    INTEGER(4),     INTENT(IN) :: visN           ! species number for viscosity
    REAL(dp),       INTENT(IN) :: Horg           ! [M/atm]
    INTEGER,        INTENT(IN) :: aerosol        ! aerosol types
  ...
  ! local vars
    ...
    REAL(dp)  :: xmms, Diff , Rp                       ! 
    REAL(dp)  :: Viscosity                             ! [N·m-2 s-1] or [Pa s]
    REAL(dp), PARAMETER :: kB = 1.380649e-23_dp        ! [N·m K-1]
    REAL(dp), PARAMETER :: Rconstant = 8.21e-02_dp     ! [L atm K-1 mol-1]
    REAL(dp)  :: lorg
    ...
    xmms = SQRT( ( 2.117e+8_dp * TEMP ) / ( srMw * srMw ) )
    ! Viscosity
    Viscosity = 10_dp ** H%Viscosity(visN) 

    ! Radius
    ! Rp   = H%xRadi(aerosol)    ! [cm]
    !!!! use the effective Rp = 1x10-10 m
    Rp = 1.0e-8_dp ![cm]
    lorg = 1.0e-1_dp * H%xRadi(aerosol)  ! [cm]

    ! Calculate diffusion coefficient [cm2/s]:
    Diff = kB * TEMP / (6.0_dp * 3.14_dp * Rp * Viscosity ) * 1.0e+6_dp
    !  
    ! Caculate gamma
    gamma = (4.0_dp * Rconstant * TEMP * Horg * Diff ) / (xmms * lorg) * 9.0e-1_dp 

    IF (gamma < 0.0_dp) gamma = 0.0_dp


    ! yumin for close the shell effect 09/19/2024
    !!!gamma = 0.0_dp

  END FUNCTION ShellGamma

! Yumin Li 18/09/2024
  FUNCTION HomoGamma(srMw, visN, Heff, oldgamma, H ) RESULT( gamma )
  ! caculate the gamma based on following assumptions:
  ! In homogenous structure
  ! The total OA change viscosity
  ! input: Rp, viscosity, Heff
  ! output: new gamma
  ! this is only for total OA effect
  ! 1/Γ_𝑎𝑞 =  (𝜔𝑅_𝑝)/(4𝑅𝑇𝐻_𝑎𝑞^∗ 〖𝑄𝐷〗_𝑎𝑞 )=𝜔/(4𝑅𝑇𝐻_𝑎𝑞^∗ √(𝐷_𝑎𝑞 𝑘_𝑎𝑞 )) (1/(coth⁡(𝑞𝑎𝑞)−1/𝑞𝑎𝑞))

    REAL(dp),       INTENT(IN) :: srMw           ! sqrt(mol wt)
    INTEGER(4),     INTENT(IN) :: visN           ! species number for viscosity
    REAL(dp),       INTENT(IN) :: Heff           ! [M/atm]
    INTEGER,        INTENT(IN) :: oldgamma       ! gamma in liquid phase

    TYPE(HetState), INTENT(IN) :: H              ! HetChem State
    REAL(dp)                   :: gamma          ! Uptake prob [1]
    REAL(dp)                   :: val1, val2, val3, qaq, kaq, Q  ! local vars
  ...
  ! local vars
    ...
    ELSE
      ...
      ! Radius
      ! Rp   = H%xRadi(aerosol)    ! [cm]
      !!!! use the effective Rp = 1x10-10 m for caculate Dorg
      Rp = 1.0e-8_dp ![cm]

      ! real radius: H%xRadi(ORC)    ! [cm]

      ! Calculate mean molecular speed [cm/s]:
      xmms = SQRT( ( 2.117e+8_dp * TEMP ) / ( srMw * srMw ) )

      ! Calculate the first uptake parameterization term:
      val1 = ( H%xRadi(ORC) * xmms ) / ( 4.0_dp * DIFF_N2O5_STD )
      !
      ! Calculate the second uptae parameterization term:
      val2 = ( 1.0_dp / MACOEFF )
      !
      ...
      ! Calculate kaq to keep the same start gamma

      kaq = 1.0_dp 

      qaq  =  H%xRadi(ORC)* SQRT (kaq / Diff)
  
      Q    =  1.0_dp / ((1.0_dp/tanh(qaq)) - 1.0_dp/qaq)

      val3 =  ( xmms ) / (4.0_dp * Rconstant * TEMP * Heff * SQRT( Diff * kaq) ) * Q 

      ! Account for small reaction rates:
      IF ( (val1 + val2 + val3) > 0 ) gamma = 1.0_dp / ( val1 + val2 + val3 )

    ENDIF


  END FUNCTION HomoGamma
...
! Yumin Li 22/07/2024
  ...
  FUNCTION IEPOXuptk1stOrd( srMw, doScale, H ) RESULT( k )
    ...
    REAL(dp)                   :: gamma          ! local vars
    REAL(dp)                   :: sgamma     ! local vars shell gamma
    REAL(dp), PARAMETER        :: Horg  = 1.7e+7_dp  ! Henry's law constant for IEPOX in mol/l/atm
    ...
    IF ( RELHUM >= CRITRH ) THEN
       ...
       IF (sgamma > 0.0_dp .AND. gamma > 0.0_dp) THEN
          gamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgamma)
       ELSEIF (sgamma > 0.0_dp) THEN
          gamma = 1.0_dp/ (1.0_dp/sgamma)
       ENDIF    
  ...
  FUNCTION MGLYuptk1stOrd( srMw, H ) RESULT( k )
    ...
    REAL(dp)                   :: gamma, k       ! rxn prob [1], rxn rate [1/s]
    REAL(dp)                   :: sgamma         ! local vars shell gamma
    REAL(dp)                   :: Horg           ! Henry's law constant for MGLY in mol/l/atm

   !------------------------------------------------------------------------------
   !Caculate the Henry's law constants by consider the salt-in and salt-out effect
   !------------------------------------------------------------------------------

      REAL(dp)              :: KHw                ! T depended Henry constant for MGLY in pure water
      REAL(dp),  PARAMETER  :: HM  = 3.24e+4_dp   ! [M atm-1]
      REAL(dp),  PARAMETER  :: DhM = 6200e+0_dp   ! [K]
      !--------------------------------------
      ! salting constant for methyglyoxal [m-1]
      REAL(dp), PARAMETER :: KsCl   = 0.06_dp
      REAL(dp), PARAMETER :: KsSO4  = 0.16_dp
      REAL(dp), PARAMETER :: KsNIT  = 0.075_dp
      !---------------------------------------
    ...
    IF ( RELHUM >= CRITRH ) THEN
       ...
       ! Henry's constant for aerosol [M atm-1]
       KHw= HM*exp(DhM*((1.e+0_dp/TEMP)-(1.e+0_dp/298.e+0_dp)))
  ...
  FUNCTION VOCuptk1stOrd( srMw, gamma,  H ) RESULT( k )
    ...
    TYPE(HetState), INTENT(IN) :: H              ! Hetchem State
    REAL(dp)                   :: k              ! rxn rate [1/s]
    REAL(dp)                   :: Horg           ! Henry's law constant for VOCs in mol/l/atm
    REAL(dp)                   :: sgammaSUL      ! local vars shell gamma
    REAL(dp)                   :: sgammaBKC      ! local vars shell gamma
    REAL(dp)                   :: sgammaORC      ! local vars shell gamma
    REAL(dp)                   :: sgammaSSA      ! local vars shell gamma
    REAL(dp)                   :: sgammaSSC      ! local vars shell gamma
    REAL(dp)                   :: sgammaSLA      ! local vars shell gamma
    REAL(dp)                   :: sgammaIIC      ! local vars shell gamma
    REAL(dp)                   :: sgamma         ! local vars   
    ...
    IF ( RELHUM >= CRITRH ) THEN
       ...
       Horg = 1.0e+8_dp*exp(7200_dp*((1.e+0_dp/TEMP)-(1.e+0_dp/298.e+0_dp)))  !!! need check later

       ! SUL 
           sgammaSUL = ShellGamma(srMw, 5, Horg, SUL, H )

       IF (sgammaSUL > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaSUL)
       ENDIF
   
       k = k + Ars_L1k( H%xArea(SUL), H%xRadi(SUL), sgamma, srMw )

       ! BKC
           sgammaBKC = ShellGamma(srMw, 5, Horg, BKC, H )

       IF (sgammaBKC > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaBKC)
       ENDIF

       k = k + Ars_L1k( H%xArea(BKC), H%xRadi(BKC), sgamma, srMw )

       ! ORC
           sgammaORC = ShellGamma(srMw, 5, Horg, ORC, H )

       IF (sgammaORC > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaORC)
       ENDIF
       k = k + Ars_L1k( H%xArea(ORC), H%xRadi(ORC), sgamma, srMw )

       ! SSA
           sgammaSSA = ShellGamma(srMw, 5, Horg, SSA, H )

       IF (sgammaSSA > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaSSA)
       ENDIF
       k = k + Ars_L1k( H%xArea(SSA), H%xRadi(SSA), sgamma, srMw )

       ! SSC
           sgammaSSC = ShellGamma(srMw, 5, Horg, SSC, H )

       IF (sgammaSSC > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaSSC)
       ENDIF
       k = k + Ars_L1k( H%xArea(SSC), H%xRadi(SSC), sgamma, srMw )
     
       ! SLA
           sgammaSLA = ShellGamma(srMw, 5, Horg, SLA, H )

       IF (sgammaSLA > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaSLA)
       ENDIF
       k = k + H%xArea(SLA) * sgamma

       ! IIC
           sgammaIIC = ShellGamma(srMw, 5, Horg, IIC, H )

       IF (sgammaIIC > 0.0_dp .AND. gamma > 0.0_dp) THEN
           sgamma = 1.0_dp/ (1.0_dp/gamma + 1.0_dp/sgammaIIC)
       ENDIF
       k = k + Ars_L1k( H%xArea(IIC), H%xRadi(IIC), sgamma, srMw )

    ENDIF

  END FUNCTION VOCuptk1stOrd

  FUNCTION VOCuptk1stOrdA( srMw, gamma,  H ) RESULT( k )
    !
    ! Computes the rxn rate [1/s]for 1st-order uptake of several VOC species.
    !
    REAL(dp),       INTENT(IN) :: srMw, gamma    ! sqrt( mol wt ), rxn prob
    TYPE(HetState), INTENT(IN) :: H              ! Hetchem State
    REAL(dp)                   :: k              ! rxn rate [1/s]
    REAL(dp)                   :: KHw            ! T depended Henry constant for in pure water at T0
    REAL(dp)                   :: HT             ! [K] T depended constant 
    REAL(dp)                   :: Horg           ! Henry's law constant for VOCs in mol/l/atm
    REAL(dp)                   :: sgammaSUL      ! local vars shell gamma
    REAL(dp)                   :: sgammaBKC      ! local vars shell gamma
    REAL(dp)                   :: sgammaORC      ! local vars shell gamma
    ...
    IF ( RELHUM >= CRITRH ) THEN
        ...
        sgamma = gamma
        Horg = 2.3e+3_dp  !!! need check later use the nitrophenol here


       ! SUL
           sgammaSUL = ShellGamma(srMw, 6, Horg, SUL, H )
       ...
       ! BKC
           sgammaBKC = ShellGamma(srMw, 6, Horg, BKC, H )
       ...
       ! ORC
           sgammaORC = ShellGamma(srMw, 6, Horg, ORC, H )
       ...
       ! SSA
           sgammaSSA = ShellGamma(srMw, 6, Horg, SSA, H )
       ...
       ! SSC
           sgammaSSC = ShellGamma(srMw, 6, Horg, SSC, H )
       ...
       ! SLA
           sgammaSLA = ShellGamma(srMw, 6, Horg, SLA, H )
       ...
       ! IIC
           sgammaIIC = ShellGamma(srMw, 6, Horg, IIC, H )
    ...
    ENDIF
  END FUNCTION VOCuptk1stOrdA

  FUNCTION VOCuptk1stOrdB( srMw, gamma, H0, HCR, H ) RESULT( k )
    !
    ! Computes the rxn rate [1/s]for 1st-order uptake of several VOC species.
    !
    REAL(dp),       INTENT(IN) :: srMw, gamma    ! sqrt( mol wt ), rxn prob
    ...
    REAL(dp)                   :: k              ! rxn rate [1/s]
    REAL(dp)                   :: Horg           ! Henry's law constant for VOCs in mol/l/atm
    REAL(dp)                   :: H0             ! Henry's law constant at T0
    REAL(dp)                   :: HCR            ! parameter for T dependence of H
    REAL(dp)                   :: sgammaSUL      ! local vars shell gamma
    REAL(dp)                   :: sgammaBKC      ! local vars shell gamma
    REAL(dp)                   :: sgammaORC      ! local vars shell gamma
    REAL(dp)                   :: sgammaSSA      ! local vars shell gamma
    ...
    IF ( RELHUM >= CRITRH ) THEN
       ...
       sgamma = gamma
       Horg = H0 * exp(HCR * ( 1.0_dp/TEMP - INV_T298 ) )
