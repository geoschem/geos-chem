! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! Initialization File
! 
! Generated by KPP-3.5.0 symbolic chemistry Kinetics PreProcessor
!       (https:/github.com/KineticPreProcessor/KPP)
! KPP is distributed under GPL, the general public licence
!       (http://www.gnu.org/copyleft/gpl.html)
! (C) 1995-1997, V. Damian & A. Sandu, CGRER, Univ. Iowa
! (C) 1997-2022, A. Sandu, Michigan Tech, Virginia Tech
!     With important contributions from:
!        M. Damian,   Villanova University, Philadelphia, PA, USA
!        R. Sander,   Max-Planck Institute for Chemistry, Mainz, Germany
!        M. Long,     Renaissance Fiber, LLC, North Carolina, USA
!        H. Lin,      Harvard University, Cambridge, MA, USA
!        R. Yantosca, Harvard University, Cambridge, MA, USA
!        P.O. Sturm,  Univ. of Southern California, Los Angeles, CA, USA
! 
! File                 : gckpp_Initialize.F90
! Equation file        : gckpp.kpp
! Output root filename : gckpp
! 
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~



MODULE gckpp_Initialize

  USE gckpp_Parameters, ONLY: dp, NVAR, NFIX
  IMPLICIT NONE

CONTAINS


! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
! 
! Initialize - function to initialize concentrations
!   Arguments :
!      PassiveSpc_ATOL_Threshold- Species with ATOL > this value are passive
! 
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

SUBROUTINE Initialize ( PassiveSpc_ATOL_Threshold )


  USE gckpp_Global

  USE gckpp_Parameters

! PassiveSpc_ATOL_Threshold - Species with ATOL > this value are passive
  REAL(kind=dp), OPTIONAL :: PassiveSpc_ATOL_Threshold

! ~~~ Local variables
  INTEGER :: i
  REAL(kind=dp) :: x

! ~~~ Define scale factor for units
  CFACTOR = 1.000000e+00_dp

! ~~~ Zero C array
  C = 0.0_dp

! ~~~ Set initial species concentrations

! Begin constant rate coefficients
  RCONST(128) = 4.51e-12
  RCONST(737) = 7.09e-11
! End constant rate coefficients

! Begin inlined code from F90_INIT

! End inlined code from F90_INIT

! ~~~ Determine the count and indices of actual species (as opposed to
! ~~~ (passive species used for diagnostics.  Denote passive species
! ~~~ as those having ATOL > PassiveSpc_ATOL_Threshold.  If the optional
! ~~~ argument PassiveSpc_ATOL_Threshold is not specified, then treat
! ~~~ all variable species as non-passive species.
  NonPassiveSpc_Count   = 0
  NonPassiveSpc_Indices = 0
  IF ( PRESENT( PassiveSpc_ATOL_Threshold ) ) THEN
     DO I = 1, NVAR
        IF ( ATOL(I) < PassiveSpc_ATOL_Threshold ) THEN
           NonPassiveSpc_Count = NonPassiveSpc_Count + 1
           NonPassiveSpc_Indices(NonPassiveSpc_Count) = I
        ENDIF
     ENDDO
  ELSE
     NonPassiveSpc_Count = NVAR
     DO I = 1, NVAR
        NonPassiveSpc_Indices(I) = I
     ENDDO
  ENDIF

      
END SUBROUTINE Initialize

! End of Initialize function
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~



END MODULE gckpp_Initialize

