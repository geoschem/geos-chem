...
CONTAINS
  ...
  ! NOTE: Will peel this out soon
  REAL(dp) FUNCTION GC_ARR( A0,B0,C0 )
      REAL A0,B0,C0
      GC_ARR =  DBLE(A0) * EXP(DBLE(C0)/TEMP) * (300._dp/TEMP)**DBLE(B0)
  END FUNCTION GC_ARR

  ! Will peel this out soon
  REAL(dp) FUNCTION GC_ARR2( A0,B0,C0 )
 ! Low-pressure limit rate coefficient for a
 ! three-body reaction
      REAL A0,B0,C0
      GC_ARR2 =  GC_ARR( A0,B0,C0 )*NUMDEN
  END FUNCTION GC_ARR2
...
!	((1+((LOG10(k0*NUMDEN/ki))**2d0)**-1.0d0))
 ...
 ! Find the backwards reaction by K=kforward/kbackwards
	  ...
	  ENDIF
       ELSE
          R1 = gc_arr( A1,B1,C1 ) !Std. Arrhenius eqn.
       ENDIF
...
!
       ...
       RLOW  = GC_ARR( A0,B0,C0 )*NUMDEN
       RHIGH = GC_ARR( A1,B1,C1 )
