...
!
      ...
      USE LOGICAL_MOD,    ONLY : LTOMAS    !(win, 1/25/10)
...
#     include "CMN_SIZE"     ! Size parameters 
      ...
      ! SO4 from aqueous chemistry of SO2 (in-cloud oxidation)  !(win, 1/25/10)
      ! 
      ! SO4 produced via aqueous chemistry is distributed onto 30-bin aerosol
      ! by TOMAS subroutine AQOXID  
      ! NOTE: This may be moved to tomas_mod.f in the future, but for now
      ! it still need to get the PSO4_SO2AQ value while CHEMSULFATE is called
      IF ( LTOMAS ) THEN 
         CALL CHEM_SO4_AQ
...
!------------------------------------------------------------------------------
     ...
     &               ( (SO2_ss * L2) - H2O20 )  
         ...
         PSO4_SO2AQ(I,J,L) = L2S + L3S  !For microphysics (win, 1/25/10)
...
!------------------------------------------------------------------------------
      ...
      SUBROUTINE CHEM_SO4_AQ
!
!******************************************************************************
!  Subroutine CHEM_SO4_AQ takes the SO4 produced via aqueous chemistry of SO2
!  and distribute onto the size-resolved aerosol number and sulfate mass as a
!  part of the TOMAS aerosol microphysics module (win, 1/25/10)
!
!  NOTES:
...
#     include "CMN_SIZE"     ! Size parameters
      ...
      DO I = 1, IIPAR
         ...
         IF ( SO4OXID > 0d0 ) THEN
            ...
            CALL GETACTBIN( I, J, L, IDTNK10, .TRUE. , BINACT1 )
            CALL GETACTBIN( I, J, L, IDTNK10, .FALSE., BINACT2 )
            KMIN = ( BINACT1 + BINACT2 )/ 2.
            CALL AQOXID( SO4OXID, KMIN, I, J, L )
         ENDIF
      ENDDO
      ENDDO
      ENDDO
...
!
      ...
      USE TOMAS_MOD,         ONLY : NH4BULKTOBIN, SRTNH4  !(win, 1/25/10)
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      REAL*8             :: BINMASS(IIPAR,JJPAR,LLPAR,IBINS*ICOMP) !(win, 1/25/10)
      REAL*8             :: tempnh4(ibins) !(win, 1/25/10)
      INTEGER            :: TID, I, J, L   !(win, 1/25/10)
...
c         print *, I,J,L,'NH4',STT(I,J,L,IDTNH4)
...
!
!******************************************************************************
!  Subroutine SRCSF30 handles 30bin sulfate SF1-SF30 and number NK1-NK30 
!   emissions into the GEOS-CHEM tracer array.(win, 1/25/10)
!
!  Arguments as Input/Output
!  ============================================================================
!  (1 ) TC1 (REAL*8 ) : Array for 30-bin NK1-NK30 tracer number [no.]
!  (2 ) TC2 (REAL*8)  : Array for 30-bin SF1-SF30 tracer mass [kg] 
!
!  Variable comments
!  ============================================================================
...
#     include "CMN_GCTM"     ! SCALE_HEIGHT
      ...
      ! Local variables
      LOGICAL                :: WEEKDAY
      INTEGER                :: I, J, K, L, DAY_NUM, NTOP, C
      INTEGER                :: NB
      ...
      REAL*8 BFRAC(30)     ! Mass fraction emitted to each bin
      DATA BFRAC/1.728d-02, 2.648d-02, 3.190d-02, 3.024d-02, 2.277d-02,
...
!!$OMP PARALLEL DO
!!$OMP+DEFAULT( SHARED )
!!$OMP+PRIVATE( I, J, NTOP, SO4, TSO4, L, FEMIS, EFRAC, N0, M0 )
!!$OMP+PRIVATE( NDISTINIT, NDIST, NDISTFINAL, NADDFINAL )
!!$OMP+SCHEDULE( DYNAMIC )
      DO J = 1, JJPAR
      DO I = 1, IIPAR
         ...
         ENDDO ! L loop
           ...
           !==============================================================
            IF ( ND59 > 0 ) THEN 
               DO L = 1, LLPAR
               DO NB = 1, IBINS
                  if(TC2(I,J,L,NB)-M0(L,NB) < 0d0) 
     &              print *,'Negative SF emis ',TC2(I,J,L,NB)-M0(L,NB),
     &                 'at',I,J,L,NB
                  if(TC1(I,J,L,NB)-N0(L,NB) < 0d0) 
     &              print *,'Negative NK emis ',TC1(I,J,L,NB)-N0(L,NB),
     &                 'at',I,J,L,NB
                  AD59_SULF(I,J,1,NB) = AD59_SULF(I,J,1,NB) + 
     &                                  (TC2(I,J,L,NB)-M0(L,NB))*S_SO4
                  AD59_NUMB(I,J,1,NB) = AD59_NUMB(I,J,1,NB) + 
     &                                  TC1(I,J,L,NB)-N0(L,NB)
               ENDDO
               ENDDO
            ENDIF
         ...
         !=============================================================
            DO L = 1, LLPAR
               SO4(L) = TSO4 * EFRAC(L)
               DO NB = 1, IBINS
                  TC1(I,J,L,NB) = TC1(I,J,L,NB) + 
     &                    ( SO4(L) * DTSRCE * BFRAC(NB) / AVGMASS(NB) )
                  TC2(I,J,L,NB) = TC2(I,J,L,NB) + 
     &                    ( SO4(L) * DTSRCE * BFRAC(NB)               )
               ENDDO
            ENDDO
           ...
           !==============================================================
            IF ( ND59 > 0 ) THEN 
               ...
               DO L = 1, 2
               DO NB = 1, IBINS
                  AD59_SULF(I,J,L,NB) = AD59_SULF(I,J,L,NB) + 
     &                                ( SO4anbf(I,J,L) * BFRAC(NB) 
     &                                    * S_SO4 * DTSRCE         )
                  AD59_NUMB(I,J,L,NB) = AD59_NUMB(I,J,L,NB) + 
     &                                ( SO4anbf(I,J,L) * BFRAC(NB) 
     &                                    / AVGMASS(NB) * DTSRCE   )
               ENDDO
               ENDDO
...
!$OMP+PRIVATE( I, J, X, Y, EDG_SO2, Fe, LL ) 
     ...
     $                                          IDTSO2, KG_S=.TRUE. )
               ...
               IF ( IDTNK1 > 0 ) THEN 
                  ...
                  !----------------------------------------------
                  ! when TOMAS aerosol is in use, switch to use 
                  ! 1% SO4 fraction (win, 1/25/10)
                  Fe = 0.01d0 
...
! 
      ...
      IF ( ALLOCATED( PSO4_SO2AQ) ) DEALLOCATE( PSO4_SO2AQ) !(win, 1/25/10)
