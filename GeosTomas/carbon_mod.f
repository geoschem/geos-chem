...
!  GOCART model and modified accordingly. (rjp, bmy, 4/2/04, 1/25/10)
!
...
#     include "CMN_SIZE"       ! Size parameters
     ...
     &                     STT(:,:,:,IDTECOB1:IDTECOB1+IBINS-1) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBO: AGING_CARB EC' )
      ENDIF
      IF ( IDTOCOB1 > 0 ) THEN
         CALL AGING_CARB( STT(:,:,:,IDTOCIL1:IDTOCIL1+IBINS-1), 
     &                     STT(:,:,:,IDTOCOB1:IDTOCOB1+IBINS-1) )
         IF ( LPRT ) CALL DEBUG_MSG( '### CHEMCARBO: AGING_CARB OC' )
      ENDIF
...
!
!******************************************************************************
!  Subroutine AGING_CARB converts the size-resolved hydrophobic EC or OC to 
!  hydrophilic EC or OC with an assumed e-folding time. (win, 9/11/07)
!
!  NOTES:
!******************************************************************************
...
!
      ...
      !USE TRACERID_MOD
      USE DIAG_MOD,    ONLY : AD59_ECIL,   AD59_ECOB  !(win, 1/25/10)
      USE DIAG_MOD,    ONLY : AD59_OCIL,   AD59_OCOB !(win, 1/25/10)
      USE DIAG_MOD,    ONLY : AD59_NUMB   !(win, 1/25/10)
      USE TOMAS_MOD,   ONLY : IBINS, AVGMASS,  SOACOND, ICOMP, IDIAG  !(win, 1/25/10)
      USE TRACERID_MOD,ONLY : IDTECIL1, IDTECOB1, IDTNK1  !(win, 1/25/10)
      USE TRACERID_MOD,ONLY : IDTOCIL1, IDTOCOB1  !(win, 1/25/10)
...
!         win, 12/05/08
 ...
 !temp debug            if(sum(EC30SRC(ii,jj,:,:)) > 0d0) dbg = .true.
      if( dbg) then
         ...
         do k=1,icomp-idiag
            print *,'comp',k
            print *,STT(ii,jj,ll,IDTNK1+k*30:IDTNK1+IBINS-1+k*30)
         enddo
      endif
!temp debug --------
...
!
!******************************************************************************
!  Subroutine EMITSGC calculates sub-grid coagulation for the size distribution
!  of emission. (win, 10/6/07)
!
!******************************************************************************
      ...
      USE TOMAS_MOD,    ONLY : SRTECIL,  SRTECOB,  SRTOCIL, SRTOCOB
      USE TOMAS_MOD,    ONLY : SRTSO4,   SRTNH4,   SRTH2O
      USE TOMAS_MOD,    ONLY : SUBGRIDCOAG,   MNFIX, NH4BULKTOBIN
...
#     include "CMN_DIAG"  ! ND59
      ...
      REAL*8                 :: TSCALE, BOXVOL, TEMP, PRES
      INTEGER                :: I, J, L, K, C, PBL_MAX
      REAL*8                 :: F_OF_PBL
      LOGICAL                :: ERRORSWITCH, PDBUG
      REAL*4                 :: N0(LLPAR,IBINS), N1(LLPAR,IBINS)
      REAL*4                 :: MIL0(LLPAR,IBINS), MIL1(LLPAR,IBINS)
      REAL*4                 :: MOB0(LLPAR,IBINS), MOB1(LLPAR,IBINS)
      integer                :: ii, jj, ll
      logical                :: dbg = .false.
      data ii, jj, ll /53, 29, 8 /
...
!     &                     PRES, TSCALE, NDISTFINAL, MADDFINAL   ) 
     &                     PRES, TSCALE, NDISTFINAL, MADDFINAL,pdbug)          
         IF ( PDBUG ) THEN
            ...
            do k=1,8 
               print *,'Mk comp',k
               print *,STT(I,J,L,IDTNK1+k*30:IDTNK1+IBINS-1+k*30)
            enddo
         ENDIF
ccc            
         DO K = 1, IBINS
...
!
!******************************************************************************
!  Function SCALECARB split the carbonaceous emission from each source into the
!  TOMAS aerosol size bins using different mass distribution for fossil fuel
!  and biomass burning+biofuel.  The mass size distributions are different for
!  EC and OC.   (win, 9/4/07)
...
#     include "CMN_SIZE"    ! Size parameters
      ...
      DATA OCSCALE30/1.04E-03,2.77E-03,6.60E-03,1.41E-02,2.69E-02,
     & 4.60E-02,7.06E-02,9.69E-02,1.19E-01,1.31E-01,1.30E-01,1.15E-01,
     & 9.07E-02,6.44E-02,4.09E-02,2.33E-02,1.19E-02,5.42E-03,2.22E-03,
     & 8.12E-04,2.66E-04,7.83E-05,2.06E-05,4.86E-06,1.03E-06,1.94E-07,
     & 3.29E-08,4.99E-09,6.79E-10,8.26E-11/ ! use for fossil fuel
      DATA OCSCALE100/3.2224e-007,1.6605e-006,7.6565e-006,3.1592e-005,
     & 0.00011664,0.00038538,0.0011394,0.0030144,0.0071362,0.015117,
     & 0.028657,0.048612,0.073789,0.10023,0.12182,0.1325,0.12895,
     & 0.11231,0.087525,0.061037,0.038089,0.02127,0.010628,0.0047523,
     & 0.0019015,0.00068081,0.00021813,6.2536e-005,1.6044e-005,
     & 3.6831e-006/ ! use for biomass burning
      DATA ECSCALE30/1.04E-03,2.77E-03,6.60E-03,1.41E-02,2.69E-02,
     & 4.60E-02,7.06E-02,9.69E-02,1.19E-01,1.31E-01,1.30E-01,1.15E-01,
     & 9.07E-02,6.44E-02,4.09E-02,2.33E-02,1.19E-02,5.42E-03,2.22E-03,
     & 8.12E-04,2.66E-04,7.83E-05,2.06E-05,4.86E-06,1.03E-06,1.94E-07,
     & 3.29E-08,4.99E-09,6.79E-10,8.26E-11/ ! use for fossil fuel
      DATA ECSCALE100/3.2224e-007,1.6605e-006,7.6565e-006,3.1592e-005,
     & 0.00011664,0.00038538,0.0011394,0.0030144,0.0071362,0.015117,
     & 0.028657,0.048612,0.073789,0.10023,0.12182,0.1325,0.12895,
     & 0.11231,0.087525,0.061037,0.038089,0.02127,0.010628,0.0047523,
     & 0.0019015,0.00068081,0.00021813,6.2536e-005,1.6044e-005,
...
!
!******************************************************************************
!  Subroutine EMITHIGH2 mixes tracer completely from the surface to the PBL
!  top. This is a copy of subroutine EMITHIGH modified to work with 30-bin
!  EC and OC mass and also aerosol number.  (win, 9/4/07)
!
!  Arguments as Input:
!  ============================================================================
!  (1 ) BCSRC (REAL*8) : Array which holds Total BC (H-phobic & H-philic)
!  (2 ) OCSRC (REAL*8) : Array which holds Total OC (H-phobic & H-philic)
!  (3 ) NUMBSRC(REAL*8): Array which holds number emission from BC+OC
!
!  NOTES:
!  (1 ) Now also mix ALPH, LIMO, ALCO tracers (rjp, bmy, 7/8/04)
!  (2 ) Now reference STT from "tracer_mod.f" (bmy, 7/20/04)
...
#     include "CMN_SIZE"  ! Size parameters
      ...
      ! Arguments
      REAL*8, INTENT(IN) :: BCSRC(IIPAR,JJPAR,IBINS, 2)
      REAL*8, INTENT(IN) :: OCSRC(IIPAR,JJPAR,IBINS, 2)
