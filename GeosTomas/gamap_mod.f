! $Id: gamap_mod.f,v 1.1 2010/02/02 16:57:49 bmy Exp $
      MODULE GAMAP_MOD
!
!******************************************************************************
...
c$$$               T            = NTRAIND(N)         
c$$$
c$$$               ! Drydep flux (deposited species only)
c$$$               NAME (N,44)  = TRIM( DEPNAME(N) ) // 'df'        
c$$$               FNAME(N,44)  = TRIM( DEPNAME(N) ) // ' drydep flux'
c$$$               UNIT (N,44)  = 'molec/cm2/s'
c$$$               MWT  (N,44)  = TRACER_MW_KG(T) 
c$$$               MOLC (N,44)  = INT( TRACER_COEFF(T,1) )
c$$$               SCALE(N,44)  = 1.0e0
c$$$               INDEX(N,44)  = T + ( SPACING * 36 )
c$$$               NTRAC(44)    = NTRAC(44) + 1
c$$$
c$$$               ! For the Rn simulation, unit is kg/s (bmy, 2/22/08)
c$$$               IF ( ITS_A_RnPbBe_SIM() ) UNIT(N,44) = 'kg/s'
c$$$
c$$$               ! Drydep velocity (deposited species only)
c$$$               NN           = N + NUMDEP
c$$$               NAME (NN,44) = TRIM( DEPNAME(N) ) // 'dv'        
c$$$               FNAME(NN,44) = TRIM( DEPNAME(N) ) // ' drydep velocity'
c$$$               UNIT (NN,44) = 'cm/s'
c$$$               MWT  (NN,44) = TRACER_MW_KG(T) 
c$$$               MOLC (NN,44) = INT( TRACER_COEFF(T,1) )
c$$$               SCALE(NN,44) = 1.0e0
c$$$               INDEX(NN,44) = T + ( SPACING * 37 )
c$$$               NTRAC(44)    = NTRAC(44) + 1
c$$$            ENDDO

            ! Manually extend the set of dry dep species.  (win, 7/14/09)
            DO N = 1, NUMDEP + (ICOMP-IDIAG)* IBINS
            ...
            IF ( N <= NUMDEP ) THEN 
               ...
               NTRAC(44)    = NTRAC(44) + 1
            ELSE
               ! GEOS-CHEM tracer # for aerosol mass
               T = IDTNK1 + IBINS - 1 + ( N - NUMDEP )
               ! Drydep flux (extended deposited species)
               NAME (N,44)  = TRIM( TRACER_NAME(T) ) // 'df'        
               FNAME(N,44)  = TRIM( TRACER_NAME(T) ) // ' drydep flux'
               UNIT (N,44)  = 'molec/cm2/s'
               MWT  (N,44)  = TRACER_MW_KG(T) 
               MOLC (N,44)  = INT( TRACER_COEFF(T,1) )
               SCALE(N,44)  = 1.0e0
