! $Id: diag_pl_mod.f,v 1.2 2010/03/15 19:33:20 ccarouge Exp $
      MODULE DIAG_PL_MOD
!
!******************************************************************************
...
!  (9 ) Add array H2SO4RATE for easy reference to TOMAS module (win, 1/25/10)
...
!      
      ...
      PUBLIC :: FAM_NAME  !(win, 1/25/10)
      PUBLIC :: H2SO4RATE !(win, 1/25/10)
      ...
      REAL*8,            ALLOCATABLE :: H2SO4RATE(:,:,:)  !(win, 1/25/10)
...
!  (4 ) Modified to use prod-loss family diagnostic in kg S unit and add
!       reference to DAO_MOD for AIRVOL. (win, 8/4/09)
!  (3 ) Add calcuation for H2SO4RATE (win, 8/4/09)
!*****************************************************************************
!
      ! References to F90 modules
      USE COMODE_MOD, ONLY : CSPEC, JLOP
      USE DAO_MOD,    ONLY : AIRVOL  !(win, 8/4/09)
...
!$OMP+SCHEDULE( DYNAMIC ) 
      ...
      DO I = 1, NLONG
         ...
         IF ( JLOOP > 0 ) THEN
            ...
            ! Overwrite FAM_PL with a unit [kg S] for tracing 
            ! gas-phase sulfur species production (SO2, SO4, MSA)
            ! (win, 8/4/09)
            IF ( FAM_NAME(N) == 'PSO21' .or. FAM_NAME(N) == 'PSO22' .or.
     &           FAM_NAME(N) == 'PSO4'  .or. FAM_NAME(N) == 'PMSA' ) 
     &           FAM_PL(I,J,L,N)      = CSPEC(JLOOP,IFAM(N)) / 6.022d23
     &                                  * 32.d-3 * AIRVOL(I,J,L) * 1d6

            ! Calculate H2SO4 production rate [kg s-1] in each time step 
            ! (win, 8/4/09)
            IF ( FAM_NAME(N) == 'PSO4' ) 
     &           H2SO4RATE(I,J,L) = CSPEC(JLOOP,IFAM(N)) / 6.022d23 *
     &                              98.d-3 * AIRVOL(I,J,L) * 1d6 /
     &                              CHEMINTV

            !Debug print (win, 8/4/09)
c            if (fam_name(n) =='PSO4') 
c     &          write(888,88)i,j,l,H2so4rate(i,j,l)
c 88            format (3I3, 'H2SO4rate (kg/s)',e13.5)
...
#     include "comode.h"           ! LFAMILY, NFAMILIES
      ...
      ! Only allocate H2SO4RATE for a TOMAS simulation !(win, 7/709)
      IF ( LTOMAS ) THEN
         ALLOCATE( H2SO4RATE( IIPAR, JJPAR, LLTROP ), STAT=AS )
         IF ( AS /= 0 ) CALL ALLOC_ERR( 'H2SO4RATE' )
      ENDIF
...
!
!******************************************************************************
!  Subroutine CLEANUP_DIAG_PL deallocates all module arrays. 
!  (bmy, 7/20/04, 1/25/10)
!
!  NOTES:
!  (1) Now deallocate TOMAS array H2OSO4RATE (win, 1/25/10)
!******************************************************************************
!
      !=================================================================
      ...
      IF ( ALLOCATED( H2SO4RATE) ) DEALLOCATE( H2SO4RATE)  !(win, 1/25/10)
