! $Id: diag3.f,v 1.1 2010/02/02 16:57:50 bmy Exp $
      SUBROUTINE DIAG3                                                      
! 
!******************************************************************************
!  Subroutine DIAG3 prints out diagnostics to the BINARY format punch file 
!  (bmy, bey, mgs, rvm, 5/27/99, 1/25/10)
!
!  NOTES: 
...
! 
      ...
      USE DIAG_MOD,     ONLY : AD59_NUMB,   AD59_SULF,   AD59_SALT   !(win, 1/25/10)
      USE DIAG_MOD,     ONLY : AD59_ECIL,   AD59_ECOB  !(win, 1/25/10)
      USE DIAG_MOD,     ONLY : AD59_OCIL,   AD59_OCOB,   AD59_DUST  !(win, 1/25/10)
      USE DIAG_MOD,     ONLY : AD60_COND,   AD60_COAG,   AD60_NUCL !(win, 1/25/10)
      USE DIAG_MOD,     ONLY : AD60_AQOX,   AD60_ERROR,  AD60_SOA !(win, 1/25/10)
      USE DIAG_MOD,     ONLY : AD61
...
!               NN   = NTRAIND(N)
               ...
               IF ( N <= NUMDEP ) THEN 
                  NN  = NTRAIND(N)
               ELSE
                  NN  = ( N - NUMDEP ) + ( IDTNK1 + IBINS - 1 )
               ENDIF
 
            ENDIF

! I comment this part out b/c for TOMAS aerosol dry dep, i need to tweak
! the list to make aerosol mass species show up in the diag44 while they 
!  are not really drydep species, thus they are not in the TINDEX(44,:)
! Might need to solve this later. (win, 7/14/09)
            ! To output only the species asked in input.geos 
            ! (ccc, 5/15/09)
c$$$            MM  = 1
c$$$            MMB = 0
c$$$            DO WHILE ( MMB /= NN .AND. MM <= TMAX(44) )
c$$$               MMB = TINDEX(44,MM)
c$$$               MM  = MM + 1
c$$$            ENDDO
c$$$            
